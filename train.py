...
from config import (
    ...
    CURRICULUM_SCHEDULE, TARGET_POINT_SELECTION_SKEW_POWER # Import new configs
)
...
def get_current_crop_region_factor(current_iter, total_iters, schedule):
    """
    Calculates the curriculum crop region factor based on the current iteration.
    """
    if not schedule:
        return 1.0 

    abs_schedule = []
    for frac, val in schedule:
        abs_iter = int(frac * total_iters)
        abs_schedule.append((abs_iter, val))
    
    abs_schedule.sort(key=lambda x: x[0])

    if current_iter <= abs_schedule[0][0]:
        return abs_schedule[0][1]
    if current_iter >= abs_schedule[-1][0]:
        return abs_schedule[-1][1]

    for i in range(len(abs_schedule) - 1):
        iter_prev, factor_prev = abs_schedule[i]
        iter_next, factor_next = abs_schedule[i+1]

        if iter_prev <= current_iter < iter_next:
            if iter_next == iter_prev: 
                return factor_prev
            
            progress = (current_iter - iter_prev) / (iter_next - iter_prev)
            current_factor = factor_prev + progress * (factor_next - factor_prev)
...
def train():
    ...
    if CURRICULUM_SCHEDULE:
        print(f"Curriculum learning enabled. Schedule: {CURRICULUM_SCHEDULE}")
    else:
        print("No curriculum learning schedule defined.")
    print(f"Target point selection skew power for training: {TARGET_POINT_SELECTION_SKEW_POWER}")
    ...
    for iteration in pbar:
        ...
        current_crop_factor = get_current_crop_region_factor(iteration, TOTAL_ITERATIONS, CURRICULUM_SCHEDULE)

        kwargs_for_train_generation = {
            'augment_size': AUGMENTATION_SIZE,
            'model_input_size': MODEL_INPUT_SIZE,
            'psf_sigma': GT_PSF_SIGMA,
            'negative_prob': NEGATIVE_SAMPLE_PROB,
            'current_curriculum_crop_region_factor': current_crop_factor,
            'target_point_skew_power': TARGET_POINT_SELECTION_SKEW_POWER # Pass skew for training
        }
        ...
        pbar.set_description(f"Iter {iteration}/{TOTAL_ITERATIONS} | Batch Loss: {loss.item():.4f} | CropF: {current_crop_factor:.2f} SkewP: {TARGET_POINT_SELECTION_SKEW_POWER:.1f}")
        ...
        if iteration % VALIDATION_INTERVAL == 0:
            ...
            kwargs_for_val_generation = {
                'augment_size': AUGMENTATION_SIZE,
                'model_input_size': MODEL_INPUT_SIZE,
                'psf_sigma': GT_PSF_SIGMA,
                'negative_prob': NEGATIVE_SAMPLE_PROB,
                'current_curriculum_crop_region_factor': 1.0, # No curriculum for validation
                'target_point_skew_power': 0.0 # No skew for validation (uniform target selection)
            }
            ...
            log_message = (f"Iter [{iteration}/{TOTAL_ITERATIONS}] LR: {optimizer.param_groups[0]['lr']:.2e} CropF: {current_crop_factor:.2f} SkewP_Train: {TARGET_POINT_SELECTION_SKEW_POWER:.1f}\n"
                           f"  Train Total: {avg_train_total_loss:.4f} (KL: {avg_train_kl_loss:.4f}, BCE: {avg_train_bce_loss:.4f})\n"
                           f"  Val   Total: {avg_val_total_loss:.4f} (KL: {avg_val_kl_loss:.4f}, BCE: {avg_val_bce_loss:.4f}) (Val SkewP: 0.0)")
            print(f"\n{log_message}")
